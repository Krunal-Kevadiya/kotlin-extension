...
class CustomSnackbar private constructor(private var builder:Builder) {

    private lateinit var flashbarContainerView:CustomSnackbarContainerView
    ...
    private fun construct() {
        flashbarContainerView = CustomSnackbarContainerView(builder.activity)
        flashbarContainerView.adjustOrientation(builder.activity)
        flashbarContainerView.addParent(this)

        flashbarView = CustomSnackbarView(builder.activity)
    ...
    class Builder(internal var activity: Activity) {
        ...
        internal var iconAnimBuilder: CustomSnackAnimIconBuilder? = null

        internal var progressPosition: ProgressPosition? = null
        internal var progressTint: Int? = null

        internal var enterAnimBuilder: CustomSnackAnimBarBuilder? = null
        internal var exitAnimBuilder: CustomSnackAnimBarBuilder? = null
         ...
         */
        fun enterAnimation(builder:CustomSnackAnimBarBuilder) = apply {
         ...
         */
        fun exitAnimation(builder:CustomSnackAnimBarBuilder) = apply {
         ...
         */
        fun iconAnimation(builder:CustomSnackAnimIconBuilder) = apply { this.iconAnimBuilder = builder }
        ...
        fun build(): CustomSnackbar {
            configureAnimation()
            val flashbar = CustomSnackbar(this)
            flashbar.construct()
            return flashbar
        }
        ...
        private fun configureAnimation() {
            enterAnimBuilder = if (enterAnimBuilder == null) {
                when (gravity) {
                    TOP -> CustomSnackAnim.with(activity).animateBar().enter().fromTop()
                    BOTTOM -> CustomSnackAnim.with(activity).animateBar().enter().fromBottom()
                }
            } else {
                when (gravity) {
            ...
            exitAnimBuilder = if (exitAnimBuilder == null) {
                when (gravity) {
                    TOP -> CustomSnackAnim.with(activity).animateBar().exit().fromTop()
                    BOTTOM -> CustomSnackAnim.with(activity).animateBar().exit().fromBottom()
                }
            } else {
                when (gravity) {
    ...
    interface OnActionTapListener {
        fun onActionTapped(bar:CustomSnackbar)
    }

    interface OnBarDismissListener {
        fun onDismissing(bar:CustomSnackbar, isSwiped: Boolean)
        fun onDismissProgress(bar:CustomSnackbar, progress: Float)
        fun onDismissed(bar:CustomSnackbar, event:DismissEvent)
    }

    interface OnTapListener {
        fun onTap(flashbar:CustomSnackbar)
    }

    interface OnBarShowListener {
        fun onShowing(bar:CustomSnackbar)
        fun onShowProgress(bar:CustomSnackbar, progress: Float)
        fun onShown(bar:CustomSnackbar)
    }
}
